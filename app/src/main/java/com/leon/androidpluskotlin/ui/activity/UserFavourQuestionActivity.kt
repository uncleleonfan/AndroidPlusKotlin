...
class UserFavourQuestionActivity : BaseListViewActivity<Question>() {
    ...
    override fun startRefresh() {
        mQuestionDataSource.refreshUserFavouredQuestions(mUserId!!, object : LoadCallback<Question> {
    ...
    override fun startLoadMoreData() {
        mQuestionDataSource.getMoreUserFavouredQuestions(mUserId!!, object : LoadCallback<Question> {
            override fun onLoadSuccess(list: List<Question>) {
    ...
    override fun startLoadData() {
        initUser()
        mQuestionDataSource.getUserFavouredQuestions(mUserId!!, object : LoadCallback<Question> {
            override fun onLoadSuccess(list: List<Question>) {
                mSwipeRefreshLayout.isRefreshing = false
                adapter.replaceData(list)
            }
    ...
    override fun onListItemClick(view: View, position: Int) {
        ...
        val desc = view.findViewById<View>(R.id.mQuestionDescription)
        val question = adapter.getItem(position)
        transitionToQuestionDetail(this, question, title, desc)
    }
}
